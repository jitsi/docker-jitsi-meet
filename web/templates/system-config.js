...
{{ $DISABLE_HTTPS := .Env.DISABLE_HTTPS | default "false" | toBool -}}
...
{{ if $DISABLE_HTTPS -}}
{{ if $ENABLE_SUBDOMAINS -}}
config.websocket = 'ws://{{ $PUBLIC_URL_DOMAIN }}/' + subdir + 'xmpp-websocket';
{{ else -}}
config.websocket = 'ws://{{ $PUBLIC_URL_DOMAIN }}/xmpp-websocket';
{{ end -}}
{{ else -}}
{{ if $ENABLE_SUBDOMAINS -}}
